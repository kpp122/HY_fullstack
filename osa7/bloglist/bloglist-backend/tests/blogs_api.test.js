...
describe('when there are initially some blogs saved', () => {
    ...
    describe('blogs can be added', () => {
        ...
        beforeAll(async () => {
            ...
            const logIn = await api.post('/api/login').send(root)
        ...
        test('new blog can be added', async () => {
            ...
            expect(JSON.parse(JSON.stringify(blogsAtEnd))).toContainEqual(
                res.body
            )
        })
        ...
        test('blogs with missing author or title wont be added', async () => {
            ...
            await api.post('/api/blogs').set(authHeader).send(blog).expect(400)
    ...
    describe('blogs can be edited', () => {
        test('blog with valid id gets edited succesfully', async () => {
            ...
            const blogAtEnd = newBlogs.find(
                (blog) => blog.id === updatedBlog._id
            )
        ...
        test('blog update with invalid id returns status 400', async () => {
            ...
            await api.put(`/api/blogs/${99999}`).send(blog).expect(400)
        })
